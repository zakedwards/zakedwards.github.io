...
# then
  echo $1 | awk '{ sum=$1 ;
			...
			break;  }} if (sum<1024) printf sum "B"; }'
#else
# echo "$1"B
#fi
...
#echo "$1" | awk -v sum="$1" ' BEGIN {hum[1024^3]="Gb"; hum[1024^2]="Mb"; hum[1024]="Kb"; for (x=1024^3; x>=1024; x/=1024) { if (sum>=x) { printf "%.2f %s\n",sum/x,hum[x]; break; } } if (sum<1024) printf sum "B"; } '
